...
```{r}
prune.good_loan <- prune.misclass(tree.good_loan, best = 25)
...
## Template RandomForest
### Bagging and Random Forest

```{r}
# Creating a bagged model using all variables & a small subset of data
set.seed(42)
bag.good_loan <- randomForest(good_loan ~ ., data = train_data_balanced_no_collinearity_small_sample, mtry = 63, importance = TRUE)
yhat.bag <- predict(bag.good_loan, newdata = validation_data_balanced[small_sample, ])
...
set.seed(42)
bag.good_loan <- randomForest(good_loan ~ ., data = train_data_balanced_no_collinearity_small_sample, mtry = 12, ntree = 25, importance = TRUE)
yhat.bag <- predict(bag.good_loan, newdata = validation_data_balanced[small_sample, ])
...
```{r}
bag.boston <- randomForest(medv ~ ., data = Boston, subset = train, mtry = 12, ntree = 25) # increasing the number of trees
yhat.bag <- predict(bag.boston, newdata = Boston[-train, ])
...
```{r}
set.seed(1)
rf.boston <- randomForest(medv ~ ., data = Boston, subset = train, mtry = 6, importance = TRUE) # reducing the number of variables considered at each split: creating a random forest
